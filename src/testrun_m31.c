...
clock_t start, stop;
  ...
  //Add upsampling
  //zero pad in Fourier
 /* complex double *tempimg, *imgfft, *imgfftup, *imgup;

  fftw_plan planfft, planfftup;
  planfft = fftw_plan_dft_2d(dimx, dimy, tempimg, imgfft, FFT_FORWARD, FFTW_MEASURE);
  planfftup = fftw_plan_dft_2d(2*dimx, 2*dimy, imgfftup, imgup, FFT_FORWARD, FFTW_MEASURE);
  purify_utils_fftshift_2d(imgfftshift, imgfft, dimx, dimy);
  imgfftup = (double*)calloc(2*dimx*2*dimy, sizeof(double));
  for(int i=0; i<dimx; i++){
    for(int j=0; j<dimy; j++){
      int ii=i+dimx/2;
      int jj=j+dimy/2;
      imgfftup[ii*2*dimy+jj]=imgfft[i*dimy+j];
...
param_m1.nmeas=100;
 //Set up the chirp operator
  ...
  start = clock();
  assert(start != -1);
  purify_chirp_init(&chirpop, datachirp, 5, 1);
  ...
  start = clock();
  assert(start != -1);
    purify_chirp_convolution(&gmat, datachirp2);
  ...
  const char *file0=strcat(argv[1],"m31dirty.fits");

  purify_image_writefile(&img_copy, file0, filetype_img);
...
//Scaling constants in the diferent representation domains
  ...
  char * file1=strcat(argv[1], "/m31sara.fits");
  purify_image_writefile(&img_copy,file1, filetype_img);
  ...
  char *file2=strcat(argv[1], "/m31sarares.fits");
  purify_image_writefile(&img_copy, file2, filetype_img);
  ...
  char * file3=strcat(argv[1], "/m31saraerror.fits");
  purify_image_writefile(&img_copy,file3, filetype_img);
